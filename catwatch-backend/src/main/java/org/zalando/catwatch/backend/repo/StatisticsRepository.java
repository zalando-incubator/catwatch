...
public interface StatisticsRepository extends CrudRepository<Statistics, Long> {
	 ...
	 */
	List<Statistics> findByOrganizationNameOrderBySnapshotDateDesc(String name, Pageable pageable);
	...
	@Query("select s from Statistics s where s.organizationName = ?1 and s.snapshotDate between ?2 and ?3 order by s.snapshotDate desc")
